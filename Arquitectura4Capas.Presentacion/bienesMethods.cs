...
namespace Arquitectura4Capas.Presentacion
{
    public static class bienesMethods
    {
        public static void returnOpciones(string opcionElegida, Usuario u)
        {
            switch (opcionElegida)
            {
                case "1":
                    ...
                    userMethods.listJugadores(u, false, true, false);
                    Console.WriteLine("\n");
                    Console.WriteLine("ID:");
                    string idElegido = Console.ReadLine();
                    listBienes(idElegido, false, u, false);
                    break;
                case "2":
        ...
        public static void listBienes(string id, Boolean justShowing, Usuario u, Boolean justListingBienes)
        {
            ...
            if (lst.Capacity > 0) Console.WriteLine("Los bienes del jugador " + id + " son:");
            if (lst.Capacity > 0 && !justShowing)
            {
                ...
                Console.WriteLine("\n");
                if (!justListingBienes) userMethods.checkIfContinue(u);
